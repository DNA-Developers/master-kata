...
@Component({
  selector: 'app-sha-quiz',
  templateUrl: './sha-quiz.page.html',
  styleUrls: ['./sha-quiz.page.scss'],
})
export class ShaQuizPage implements OnInit {
  ...
  ngOnInit() {
    this.getData.getAllData().subscribe(data => {
      ...
      this.a = this.characters.slice(74, 77);
      this.list = [this.a[0]];
      this.loaded = true;
    });
  }
